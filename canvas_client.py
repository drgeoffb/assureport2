...
class CanvasClient:
    ...
    def get_all_outcomes(self, account_id: int):
        ...
        for item in all_data:
            ...
            if guid and "MAPPED_TO:" in guid:
                mapped.append(
                    {"id": outcome["id"], "title": outcome["title"], "guid": guid}
                )
            else:
    ...
    def search_outcomes_recursive(self, account_id: int, query: str):
        ...
        for item in links:
            ...
            if query in title.lower():
                results.append(
                    {"id": outcome.get("id"), "title": title, "account_name": acc_name}
                )
    ...
    def map_outcome_multi(self, outcome_id: int, parent_id: int, parent_title: str):
        ...
        assurance_footer = (
            f"<hr><p><b>Assurance Mapping:</b> This outcome aligns with:</p><ul>"
        )
    ...
    def unmap_outcome_surgical(
        self, outcome_id: int, parent_id: int, parent_title: str
    ):
        ...
        # 1. Update GUID
        ids = (
            current_guid.replace("MAPPED_TO:", "").split(",")
            if "MAPPED_TO:" in current_guid
            else []
        )
    ...
    def _format_outcome_node(self, o):
        ...
        # Priority: display_name field > title field > numeric ID
        display_name = o.get("display_name") or ref_code or f"ID: {o_id}"

        # Clean description: Take only the part before the <hr> separator
        full_desc = o.get("description") or ""
        clean_desc = full_desc.split("<hr>")[0] if "<hr>" in full_desc else full_desc

        # Process GUID for established mappings (Anti-Self Filter)
        ...
        for val in raw_vals:
            v = val.strip()
            # Keep if: Not empty AND not the numeric ID AND not the Ref Code
        ...
        for g in groups:
            if g.get("vendor_guid") == "ROOT" or g.get("title") == acc_name:
                ...
                node["children"].extend(
                    self._get_nested_group_contents(
    ...
    def _get_nested_group_contents(
        ...
        # Inside canvas_client.py -> _get_nested_group_contents

        # A. Get Outcomes for this group
        # Add the outcome_style=full parameter to the URL
        ...
        # If the account-level call fails, try the generic one with the same style parameter
        ...
        if not subgroups:
            subgroups = self._paginate(
                f"{self.base_url}/outcome_groups/{group_id}/subgroups", silent=True
            )
